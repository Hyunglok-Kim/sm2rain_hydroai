"""
01_SM2RAIN.py  (기본 3-매개변수 SM2RAIN, KST)
===============================================
ASCAT 토양수분으로부터 Brocca et al. (2014) 기본식(GA 변형 아님)으로
강수를 역산한다.

  물수지식  : Z*dtheta/dt = P - a*theta^b   (강우 중에는 지표유출 r,
                                              증발산 e를 무시)
  강수 역산 : P(t) = Z*(dtheta/dt) + a*theta(t)^b ,  P<0 -> 0
  3개 매개변수: a(배수 계수), b(배수 지수), Z(유효 토양층 깊이, mm)

입력 : data/ASCAT_daily_stack_KST.nc  (원시 스택, 결측 보간 없음, KST 일경계)
       - sm_volumetric (y, x, time), 픽셀별 0-1 정규화하여 theta로 사용
       data/da_IDWs.nc  (IDW_AWS 일강수, 보정 기준자료)
보정 : 2021년 IDW_AWS(KST) 기준 픽셀별 최적화 후 2022-2025에 적용
출력 : output/SM2RAIN_KST.nc  (time, lat, lon / mm/day) + a, b, Z 매개변수 지도

* ASCAT 격자(58x46) -> AWS/SM2RAIN 격자(49x49) 재격자화는 numpy 기반
  NaN-aware 이중선형 보간으로 수행한다(이 단계에는 scipy 불필요).
  결측 보간된 IF_KST 버전이 아닌 원시 스택을 그대로 사용한다.
"""
...
import pandas as pd
import xarray as xr
from scipy.optimize import minimize   # 최적화에만 사용 (설치 필요)
...
ASCAT_PATH = os.path.join(DATA_DIR, "ASCAT_daily_stack_KST.nc")   # 원시 스택 (결측 보간 없음)
AWS_IDW_PATH = os.path.join(DATA_DIR, "da_IDWs.nc")               # IDW_AWS 보정 기준자료
OUTPUT_PATH = os.path.join(OUTPUT_DIR, "SM2RAIN_KST.nc")
PARAM_PATH = os.path.join(OUTPUT_DIR, "SM2RAIN_params_KST.nc")

YEARS = [2021, 2022, 2023, 2024, 2025]
CAL_YEAR = 2021
TEST_START_YEAR = 2022
MIN_VALID = 30
REUSE_SAVED_PARAMS = True   # True: 저장된 a,b,Z가 있으면 재사용 (재최적화 생략)

# 매개변수 순서: a, b, Z  (원본 basic SM2RAIN_precipitation.nc와 동일한 경계)
#   a : 배수 계수
#   b : 배수 비선형 지수
#   Z : 유효 토양층 깊이 [mm] (저류 변화항 Z*dtheta/dt)
...
# ============================================================
# ASCAT -> AWS 격자 numpy 이중선형 보간 (NaN-aware, scipy 불필요)
# ============================================================
...
def regrid_bilinear(cube, src_lat, src_lon, tgt_lat, tgt_lon, min_cov=0.5):
    """cube (nlat,nlon,T) -> (TL,TO,T). 해양/결측 NaN은 유효 가중치
    재정규화로 제외한다."""
...
# IDW_AWS 로딩
# ============================================================
# 단일 파일(da_IDWs.nc: 변수 'AWS (IDW)', 2차원 y/x 좌표, 차원 time,y,x)을
# 한 번만 열어 캐시한다. 파일은 수정하지 않으며, 배열과 좌표를
# (lat, lon, time)으로 재구성하고 연도별로 잘라 쓰기만 한다.
_AWS_DS_CACHE = None
...
def load_aws_year(yr):
    ...
    # AWS 변수를 명시적으로 선택 ('aws' 포함, ASOS 제외)
    ...
    # 단일 파일에서 요청 연도만 잘라낸다 (연도별 배열은 나중에
    # 연도 순서로 이어 붙임).
    tvals = pd.DatetimeIndex(ds["time"].values)
    ymask = (tvals.year == yr)
    arr = arr[:, :, ymask]
    print(f"  IDW_AWS_{yr}: {tuple(da.shape)} {da.dims} -> {arr.shape}  ({int(ymask.sum())} days)")
...
def sm2rain_basic(sm, a, b, Z):
    """P(t) = Z*(theta_t - theta_{t-1}) + a*theta_t^b ,  음수는 0으로 절단
    (역산의 물리적 제약)."""
...
def load_saved_params(lat, lon):
    """저장된 a,b,Z를 읽는다. 전용 파일이 없으면 메인 출력 파일에서 시도."""
...
# ============================================================
print("=" * 60)
print("SM2RAIN (basic 3-param): loading data")
print("=" * 60)
...
out_idx = np.where(time_pd.year >= CAL_YEAR)[0]      # 저장 범위: 2021(학습)~2025 전체

# ASCAT SM -> AWS 격자 (numpy 이중선형). 오름차순 목표 좌표로 보간한 뒤
# 배열 순서를 원래 좌표 방향에 맞춰 되돌린다.
tl = np.sort(aws_lat_1d); to = np.sort(aws_lon_1d)
def to_aws(t_idx):
    rg = regrid_bilinear(SM_norm[:, :, t_idx], lat_1d, lon_1d, tl, to)
    # AWS 좌표가 내림차순이면 다시 뒤집는다
...
# Auto-detect IDW_AWS flip (nearest to valid-region centroid)
...
SM_cal_aws = _sm_cal
SM_out_aws = to_aws(out_idx)          # all of 2021~2025 (for estimation/saving)
print(f"  SM_cal_aws {SM_cal_aws.shape}  SM_out_aws {SM_out_aws.shape}")
if SM_cal_aws.shape != P_cal.shape:
    raise ValueError(f"grid mismatch: SM_cal={SM_cal_aws.shape}, P_cal={P_cal.shape}")

# ============================================================
# 픽셀별 매개변수 준비: 저장값 재사용 또는 2021년으로 보정
# ============================================================
print("\nPreparing per-pixel SM2RAIN(basic) parameters")
...
if saved_params is not None:
    ...
    print("  skipping re-optimization: using saved fixed a,b,Z")
    if not os.path.exists(PARAM_PATH):
        save_params(PARAM_PATH, a_map, b_map, Z_map, aws_lat_1d, aws_lon_1d)
else:
    print("  no saved parameters or reuse disabled -> optimizing anew on 2021")
    ...
    print(f"  pixels to optimize: {valid_mask.sum()} / {lat_len * lon_len}")

    for i in tqdm(range(lat_len), desc="  lat rows"):
    ...
    print(f"  calibration succeeded: {np.isfinite(a_map).sum()} pixels")
    save_params(PARAM_PATH, a_map, b_map, Z_map, aws_lat_1d, aws_lon_1d)
...
# ============================================================
# 2021~2025 적용 (보정 성공 픽셀만)  * 2021은 학습(in-sample) 연도
# ============================================================
print("\nSM2RAIN(basic) precipitation estimation (2021~2025)")
...
print(f"  valid estimates: {np.isfinite(P_sm2rain).sum()}")
...
# NFS에서는 기존/손상(0바이트) 파일을 덮어쓸 때 netCDF4 권한 오류가 날 수
# 있으므로, 새로 쓰기 전에 기존 파일을 먼저 삭제한다.
if os.path.exists(OUTPUT_PATH):
    os.remove(OUTPUT_PATH)
ds_out.to_netcdf(OUTPUT_PATH)
print("  done:", ds_out["precipitation"].shape, "(time, lat, lon)")
