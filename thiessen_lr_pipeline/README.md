# 티센 목표 선형회귀 강우 산정 (LR-G)

편의보정의 **목표자료를 지상관측 내삽장(IDW_AWS)에서 수문 실무의 유역강우 산정
관행인 티센 유역평균으로 바꾼** 최종 산출물 파이프라인이다. 지점 시자료로 티센
유역 일강수를 만들고, 그것을 목표로 두 가지 선형회귀 산출물을 낸 뒤 각각 다른
관점에서 평가한다.

```
01_thiessen_basin.py     ─┐  지점 시자료 → 티센 유역 일강수      (목표·평가기준)
02_LR_grid.py            ─┼  격자별 회귀 → 전국 격자장 LR-G     [최종 산출물]
03_LR_basin.py           ─┼  BC-G 유역평균 → 회귀 → 유역 시계열  [산출물 ②, 미계측 평가]
04_evaluate_grid.py      ─┼  LR-G 를 지상관측 산출물과 비교      [분석용]
...
| | 출발점 | 회귀 단위 | 결과 |
|---|---|---|---|
| **① `02` (LR-G)** | `SM2RAIN` `ERA5` `GPM` `TCA` `AWS`(지상관측 격자장) | 격자 한 칸씩 | 0.1° 전국 격자장 |
| **② `03`** | 이미 편의보정된 **`BC-G`** | 유역 하나씩 (면적가중 평균 후) | 표준유역 일강수 |

①은 LightGBM 편의보정(BC-G)과 **같은 입력 구성으로 목표만 티센으로 바꾼** 것이다.
회귀계수는 지상관측 격자장이 골격을 이루고(전국 중앙값 1.0 부근) 위성·재분석 네
변수가 잔차를 보정한다. 계수가 1 보다 큰 격자는 지상관측 보간이 유역 티센보다
낮게 잡는 곳으로, 회귀가 그 차이를 되돌린다 — 극한강수 재현이 BC-G 대비 크게
개선되는 것이 이 구성의 효과다. 격자 자료이므로 04 에서 지상관측 계열 산출물
(`IDW_AWS` `BC-G`)과 견준다.
...
|---|---|---|
| **무엇을** | `LR_grid` (LR-G, 최종 산출물) | `LR_basin` (산출물 ②) |
| **무엇과** | `IDW_AWS` · `BC-G` | `IDW_AWS` · `BC-G` |
| **자료 성격** | 0.1° 격자 산출물끼리 | 유역 면적가중 자료끼리 |
| **보는 것** | 일반 성능 — KGE · R · RMSE · 누적비 | 극한강우 — 연 최대일 재현비 · 강우강도 구간별 재현비 |

비교군은 이 저장소가 이미 내는 자료다.

| 이름 | 어디서 | 설명 |
|---|---|---|
| `IDW_AWS` | `ds_merged_LR.nc` 의 `AWS` | 지상관측 보간 격자장. 조밀한 관측이 도달하는 성능의 참조선 |
| `BC-G` | `sm2rain_pipeline/03_BC_LightGBM.py` 의 `BC_2` | LightGBM 편의보정. 같은 날 지상관측을 입력으로 받는 융합 산출물 (직전 단계) |
...
| 파일 | 만든 곳 | 내용 |
|---|---|---|
| **`LR_grid.nc`** | 02 | **전국 0.1° 격자 강우장 LR-G** (time × lat × lon). 격자별 회귀계수 `coef` 와 적합일수 `n_fit` 을 같이 담는다 |
