#!/usr/bin/env python3
"""02 — 격자별 선형회귀로 전국 강우 격자장(LR-G)을 만든다.  [최종 산출물]

격자 한 칸마다 선형회귀를 따로 적합하고, 그 칸의 목표는 칸이 속한 표준유역의
티센 일강수다.  입력 구성은 LightGBM 편의보정(BC-G)과 같고, 목표만 IDW_AWS
에서 수문 실무의 유역강우 산정 관행인 티센으로 바뀐다.
...
티센보다 낮게 잡는 곳으로, 회귀가 그 차이를 되돌린다.  극한강수 재현이
BC-G 대비 크게 개선되는 것이 이 구성의 효과다 (04 에서 확인한다).
...
입력
    data/ds_merged_LR.nc            SM2RAIN·ERA5·GPM·TCA·AWS 격자
